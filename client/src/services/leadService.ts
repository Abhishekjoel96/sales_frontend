...
// Get a lead by phone number (for incoming messages)
export const getLeadByPhoneNumber = async (phoneNumber: string): Promise<Lead | null> => {
    try {
        ...
        // Assuming your backend returns an array of leads, even if it's just one.
        return response.data[0] || null; // Return the first lead, or null if none.
    } catch (error: any) {
